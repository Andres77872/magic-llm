...
OPENAI_KEY = json.load(open('/home/andres/Documents/keys.json'))['groq']

OPENAI_KEY["callback"] = _get_callback
MODEL = 'llama3-8b-8192'
MODEL_FAIL = 'FAIL/llama3-8b-8192'
...
async def test_async_openai_base_stream_transcriptions_1():
    data = AudioTranscriptionsRequest(
        file=open('/home/andres/Music/speech.mp3', 'rb').read(),
...
def test_sync_openai_base_stream_transcriptions_2():
    data = AudioTranscriptionsRequest(
        file=open('/home/andres/Music/Por Qué el Superpegamento es Perfecto Para Pegar la Piel [KfxT7KuTCH4].mp3', 'rb').read(),
        model='whisper-large-v3'
    )
    k = OPENAI_KEY.copy()
    client = MagicLLM(
    ...
    content = client.llm.sync_audio_transcriptions(data)
    print('content', content)
